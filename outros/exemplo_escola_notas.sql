...
-- Calcula a Média sem arredondar
select tb_aluno.id, tb_aluno.nome, tb_aluno.turma, format(avg(tb_nota.nota), 2) as media 
from tb_aluno inner join tb_nota on tb_nota.aluno_id=tb_aluno.id 
group by tb_aluno.id, tb_aluno.nome;

-- Calcula a Média arredondando (Ceil -> arredonda o número, longe de zero, sempre para cima.)
select tb_aluno.id, tb_aluno.nome, tb_aluno.turma, format(ceil(avg(tb_nota.nota)), 2) as media 
from tb_aluno inner join tb_nota on tb_nota.aluno_id=tb_aluno.id 
group by tb_aluno.id, tb_aluno.nome;

-- Calcula a Média arredondando (Round -> arredonda o número para cima ou para baixo)
select tb_aluno.id, tb_aluno.nome, tb_aluno.turma, format(round(avg(tb_nota.nota)), 2) as media 
from tb_aluno inner join tb_nota on tb_nota.aluno_id=tb_aluno.id 
group by tb_aluno.id, tb_aluno.nome;

-- Calcula a Média arredondando (Floor -> arredonda o número, para zero, sempre para baixo)
select tb_aluno.id, tb_aluno.nome, tb_aluno.turma, format(floor(avg(tb_nota.nota)), 2) as media 
from tb_aluno inner join tb_nota on tb_nota.aluno_id=tb_aluno.id 
group by tb_aluno.id, tb_aluno.nome;